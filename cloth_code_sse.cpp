...
void initMatrix(int n, double UNUSED(mass), double UNUSED(fcon),
  ...
  *x = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *y = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *z = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *cpx = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *cpy = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *cpz = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  ...
  *fx = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *fy = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *fz = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *vx = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *vy = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *vz = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *oldfx = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *oldfy = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  *oldfz = (double *) _mm_malloc(n * n * sizeof(double), alignment);
  ...
  *ke = 0.0;
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < simd_bound; j += 4) {
      ...
      ke_vec = ADD_PD(ke_vec, ADD_PD(ADD_PD(MUL_PD(vx_vec, vx_vec), MUL_PD(vy_vec, vy_vec)), MUL_PD(vz_vec, vz_vec)));
    }
    ...
    for (ny = 0; ny < n; ny++) {
      ...
      for (dx = dx_start; dx < nx; dx++) {
        ...
        for (dy = dy_start; dy < simd_bound; dy += 4) {
          calc_interaction_avx(dx, dy, nx, ny, n, neighbour_size, delta, fcon, x, y, z, x_tmp_vec, y_tmp_vec, z_tmp_vec,
                               &fx_tmp_vec, &fy_tmp_vec, &fz_tmp_vec, &pe_vec, rlen_table);
        }
