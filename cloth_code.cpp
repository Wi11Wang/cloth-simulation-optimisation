  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      ...
      if (vmag < rball) {
        ...
        x_tmp = xdiff * rball / vmag;
        y_tmp = ydiff * rball / vmag;
        z_tmp = zdiff * rball / vmag;
        ...
        double proj_scalar = (*x_vel * xdiff / vmag + *y_vel * ydiff / vmag + *z_vel * zdiff / vmag);
        *x_vel = 0.1 * (*x_vel - xdiff / vmag * proj_scalar);
        *y_vel = 0.1 * (*y_vel - ydiff / vmag * proj_scalar);
        *z_vel = 0.1 * (*z_vel - zdiff / vmag * proj_scalar);
      }
    }
  }
